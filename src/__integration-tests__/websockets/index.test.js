...
describe('assist is connected to a websocket', () => {
  ...
  describe('received message with a speedup transaction', () => {
    let payload
    let handleEventSpy
    const transaction = {
      status: 'speedup',
    ...
    describe('eventCode is txSpeedup', () => {
      beforeAll(() => {
        payload = {
          event: { transaction, eventCode: 'txSpeedup' }
        }
      })
      test(`emitted event should have eventCode 'txSpeedup'`, () => {
        expect(handleEventSpy).toHaveBeenCalledWith({
          eventCode: 'txSpeedup',
          categoryCode: 'activeTransaction',
          transaction: txObj.transaction,
  ...
  describe('received message with a cancel transaction', () => {
    let payload
    let handleEventSpy
    const transaction = {
      status: 'cancel',
    ...
    describe('eventCode is txCancel', () => {
      beforeAll(() => {
        payload = {
          event: { transaction, eventCode: 'txCancel' }
        }
      })
      test(`emitted event should have eventCode 'txCancel'`, () => {
        expect(handleEventSpy).toHaveBeenCalledWith({
          eventCode: 'txCancel',
          categoryCode: 'activeTransaction',
          transaction: txObj.transaction,
